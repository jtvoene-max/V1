...
export function ActionPanel({ orderId, actions }: { orderId: string; actions: AtelierAction[] }) {
  ...
  return (
    <section className="rounded-lg border-2 border-neutral-800 p-4">
      <h2 className="mb-3 text-sm font-medium">Acties</h2>
      <form action={formAction} className="flex flex-col gap-3">
        <input type="hidden" name="orderId" value={orderId} />
        {needsNote && (
          <label className="flex flex-col gap-1 text-sm">
            Notitie (verplicht bij keuring)
            <textarea
              name="note"
              rows={2}
              placeholder="Bijv. hologramsticker en stiksels gecontroleerd, alles conform"
