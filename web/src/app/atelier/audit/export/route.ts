...
export async function GET(request: NextRequest) {
  ...
  const header = "tijdstip;entiteit;entiteit_id;actie;van;naar;door;door_email;notitie";
  const lines = rows.map((r) =>
    [
      ...
      csvEscape(r.toValue),
      csvEscape(r.actor?.name ?? "systeem"),
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="papertrail-${new Date().toISOString().slice(0, 10)}.csv"`,
    },
  });
}
